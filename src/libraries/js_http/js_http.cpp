...
#define JS_MAX_QUEUED_EVENTS 1000
...
int JS_http_pop_event (lua_State *L){
  ...
  struct JS_http_event e = (eq->events)[eq->start];
  if (eq->start == eq->end){
    //lua_pushnil(L);
    return 0;
  }
  else {
    lua_pushstring(L,*e.handle);
    lua_pushstring(L,*e.body);
    lua_pushstring(L,*e.headers);
    // lua_pushstring(L,e->headers);
    eq->start+=1;
    if (eq->start > JS_MAX_QUEUED_EVENTS - 1){
      eq->start=0;
    }
    return 3;
  }
